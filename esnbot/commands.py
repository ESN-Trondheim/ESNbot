...
def passord(client, channel, user, argument, output):
    if not argument:
        return client.respond_to(channel, user, "Please specify a service.")

    try:
        contact_info_sheet = gsheets.open_spreadsheet("PASSWORD_SHEET_KEY")
    except gspread.SpreadsheetNotFound:  # Error handling
        log_to_console("Spreadsheet not found...")
        client.respond_to(
            channel,
    ...
    if response:
        ...
        client.respond_to(
            channel, user, "Sorry, could not find a password for the service '" + argument[0] + "'"
        )
