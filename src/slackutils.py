    ...
    if output_list:
        for output in output_list:
            ...
            if output and 'text' in output and constants.AT_BOT in output['text']:
                text = output['text'].split(constants.AT_BOT)[1].strip()
                ...
                if output.get('user') == constants.BOT_ID:
...
def handle_command(text, channel, user, output):
    ...
    """
    if not text:
        respond_to(channel, user, "You tagged me! Try " + constants.AT_BOT + " `list` to get started.")
        return
...
def choose_command(command, arguments, channel, user, output):
    ...
    if not command in constants.COMMANDS:
        respond_to(channel, user, (
            ...
            f"Try `{constants.AT_BOT} list` or `{constants.AT_BOT} help`"
        ),
            ephemeral=True)
    func = constants.cmds[command]

    return func(channel, user, arguments, output)
